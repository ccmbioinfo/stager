...
def upgrade():
    ...
    op.add_column('dataset', sa.Column('dataset_type', sa.String(length=50), nullable=False))
    op.create_foreign_key(None, 'dataset', 'dataset_type', ['dataset_type'], ['dataset_type'])
    op.add_column('pipeline_datasets', sa.Column('supported_metadataset_type', sa.String(length=50), nullable=False))
    op.create_foreign_key(None, 'pipeline_datasets', 'metadataset_type', ['supported_metadataset_type'], ['metadataset_type'])
...
def downgrade():
    ...
    op.add_column('pipeline_datasets', sa.Column('supported_dataset', mysql.ENUM('CES', 'CGS', 'CPS', 'RES', 'RGS', 'RLM', 'RMM', 'RRS', 'RTA', 'WES', 'WGS', 'RNASeq', 'RCS', 'RDC', 'RDE'), nullable=False))
